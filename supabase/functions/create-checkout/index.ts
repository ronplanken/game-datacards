...
const CREEM_API_URL = Deno.env.get("CREEM_TEST_MODE") === "true"
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const checkoutResponse = await fetch(`${CREEM_API_URL}/v1/checkouts`, {
      ...
      body: JSON.stringify({
        productId,
        successUrl: successUrl || `${Deno.env.get("APP_URL") || "https://game-datacards.eu"}/`,
        customer: {
          email: user.email,
        },
        metadata: {
    ...
    return new Response(
      JSON.stringify({ checkoutUrl: checkoutData.checkoutUrl }),
