...
// 11th edition unit name/subname are plain strings: the loader resolves the
// top-level name to a string and the renderers read both raw (and concatenate
// them for the data attributes), so they must not become language-keyed objects.
export function UnitBasicInfo() {
  ...
  return (
    <Form>
      ...
      <Form.Item label={"Combat Patrol"}>
        <Switch
          checked={activeCard.combatPatrol || false}
          onChange={(value) => updateActiveCard({ ...activeCard, combatPatrol: value })}
        />
      </Form.Item>
