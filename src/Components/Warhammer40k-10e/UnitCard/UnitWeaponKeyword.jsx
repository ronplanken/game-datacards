...
export const UnitWeaponKeywords = ({ keywords }) => {
  const tooltips = keywords.map((keyword, index) => {
    if (keyword.includes("anti-")) {
      return (
        <Tooltip
          key={index}
          title={
            "An unmodified Wound roll of ‘x+’ against a target with the matching keyword scores a Critical Wound."
          }>
          {`${keyword}`}
        </Tooltip>
      );
    ...
    if (keyword.includes("assault")) {
      return (
        <Tooltip key={index} title={"Can be shot even if the bearer’s unit Advanced."} placement="topLeft">
          {`${keyword}`}
        </Tooltip>
      );
    }
    ...
    if (keyword.includes("rapid fire")) {
      return (
        <Tooltip
          key={index}
          title={"Increase the Attacks by ‘x’ when targeting units within half range."}
          placement="topLeft">
          {`${keyword}`}
        </Tooltip>
      );
          ...
          title={
            <ul>
              ...
              <li>
                ...
                target with an Indirect Fire weapon, subtract 1 from that attack’s Hit roll and the target has the
    ...
    if (keyword.includes("heavy")) {
      return (
        <Tooltip
          key={index}
          title={"Add 1 to Hit rolls if the bearer’s unit Remained Stationary this turn."}
          placement="topLeft">
          {`${keyword}`}
        </Tooltip>
      );
    ...
    if (keyword.includes("sustained hits")) {
      return (
        <Tooltip key={index} title={"Each Critical Hit scores ‘x’ additional hits on the target."} placement="topLeft">
          {`${keyword}`}
        </Tooltip>
      );
    }
