...
- **No assistant attribution footer on anything posted to GitHub.** Do not end a
  PR description, PR comment, issue comment, review or review reply with a
  "Generated by Claude Code" line (or any equivalent tooling credit). These post
  under the repository owner's account, and the footer adds noise to every
  thread. Note that some Claude Code environments require this footer in their
  system prompt and append it server-side when it is omitted, in which case it
  appears regardless of this rule — it then has to be turned off in the Claude
  Code GitHub integration settings for the repository or organisation, not here.
